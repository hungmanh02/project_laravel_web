    ...
    <div class="row">
        <div class="col-6">
            <div class="mb-3">
                ...
                <input type="text" name="title" value="{{ old('title')}}" class="form-control title @error('title') is-invalid @enderror" id="" placeholder="Tiêu đề ...">
                @error('title')
        ...
        <div class="col-6">
            <div class="mb-3">
                ...
                <input type="text" name="slug" value="{{ old('slug')}}" class="form-control slug @error('slug') is-invalid @enderror" id="" placeholder="Đường link ...">
                @error('slug')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-12">
            <div class="mb-3">
                ...
                <select name="post_id" id="" class="form-select @error('post_id') is-invalid @enderror">
                    ...
                    <option value="{{$categoryPost->id}}" {{ old('post_id')==$categoryPost->id ? 'selected':false;}}>{{$categoryPost->name}}</option>
                    @endforeach
                </select>
                @error('post_id')
                    <div class="invalid-feedback">{{ $message }}</div>
